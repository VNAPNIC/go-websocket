    ...
    <div class="mobile-page">
        <div class="interface">
            <div class="personnel-list">
               ...
               <!-- online list -->
                 <!-- user list
             Pull the user list when entering
             add when someone joins
             Delete after someone quits -->
            </div>
            <div class="chat-with">
                <div class="admin-group">
                    ...
                    <div class="admin-msg">
                        ...
                        <span class="admin-reply">Welcome to join the chat~</span>
                    </div>
                </div>
            </div>
            <div class="send-msg">
                <!-- <input type="text" name="msg" placeholder="The message you want to send" value="" size="35">
        ...
        // connect webSocket
        ws = new WebSocket("ws://127.0.0.1:8090/acc");

        ws.onopen = function(evt) {
            console.log("Connection open ...");

             // After connecting
             // person = prompt("Please enter your name", "hello-" + currentTime());
             // if (person != null) {
             // console.log("User is ready to log in:" + person);
             // ws.send('{"seq":"' + sendId() + '","cmd":"login","data":{"userId":"' + person + '","appId" :101}}');
             // }

            person = randomNumber(10000, 99999)
            console.log("User ready to log in:" + person);
            ws.send('{"seq":"' + sendId() + '","cmd":"login","data":{"userId":"' + person + '","appId":101}}');
        ...
        // heartbeat
        function heartbeat() {
            console.log("timed heartbeat:" + person);
        ...
        setTimeout(function() { getUserList(); }, 500); // Execute will be called after 1 second
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data.data.userList, function(i, n) {
                        //Get the value of the property optionsValue in the object
                        let name = n
                        if (n == person) {
                            name = name + "(Own)"
                        }
